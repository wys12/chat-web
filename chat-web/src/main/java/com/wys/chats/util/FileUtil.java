...
public class FileUtil {
	public final static String TEMP_FILE_TYPE_QR = "QR";
	public final static String TEMP_FILE_TYPE_ARTICLE = "article";
	 ...
	 * @param fileName 文件名
	 * @return 文件地址
	 * @throws Exception 文件找不存在报错
	 */
	public static String getTempFile(String fileName) throws Exception {
		return File.createTempFile("temp", "temp").getParent() + "\\" + fileName;
	}
	/**
	 * 读TXT文件内容
	 *
	...
	/**
	 * 将字节类型的存储容量转成带单位的存储容量
	 *
	 * @param size 字节单位存储容量
	 * @return 带单位的存储容量
	 */
	public static String sizeToString(long size) {
		return sizeToString(size, "B", "");
	}

	/**
	 * 转换存储容量单位
	 *
	 * @param size    unit单位存储容量
	 * @param unit    单位
	 * @param sizeStr 上级单位存储容量
	 * @return 带单位存储容量单位
	 */
	public static String sizeToString(long size, String unit, String sizeStr) {
		if (size >= 1024) {
			sizeStr = (size % 1024) + unit + " " + sizeStr;
			size = size / 1024;
		} else {
			return size + unit + " " + sizeStr;
		}
		switch (unit) {
		case "B":
			return sizeToString(size, "KB", sizeStr);
		case "KB":
			return sizeToString(size, "MB", sizeStr);
		case "MB":
			return sizeToString(size, "GB", sizeStr);
		case "GB":
			return sizeToString(size, "TB", sizeStr);
		case "TB":
			return sizeToString(size, "PB", sizeStr);
		case "PB":
			return sizeToString(size, "EB", sizeStr);
		default:
			return size + unit;
		}
	}
